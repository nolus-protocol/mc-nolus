...
def LP_interest_calculate(LS_Opening, LP_Deposit, LS_Repayment, LS_Liquidation, LP_Withdraw, SYS_LP_Withdraw,
    ...
    while i < len(timestamps):
        ...
        LP_Pool_State.loc[LP_Pool_State["LS_timestamp"]==timestamp,"SYS_LS_interest"] = LP_Pool_State.loc[LP_Pool_State["LS_timestamp"]==timestamp,"LP_Pool_id"].map(repayment_sum) + LP_Pool_State.loc[LP_Pool_State["LS_timestamp"]==timestamp,"LP_Pool_id"].map(liquidation_sum)
        LP_Pool_State.loc[LP_Pool_State["LS_timestamp"] == timestamp, "SYS_LS_Pool_interest"] = LP_Pool_State.loc[LP_Pool_State["LS_timestamp"]==timestamp,"SYS_LS_interest"]*(1-args["treasury_interest"]/100)
        LP_Pool_State.loc[LP_Pool_State["LS_timestamp"] == timestamp, "SYS_TR_interest"] = LP_Pool_State.loc[LP_Pool_State["LS_timestamp"]==timestamp,"SYS_LS_interest"]*(args["treasury_interest"]/100)
        #todo: SYS_LS_interest = liq_sum+rep_sum//// TVL = SYS_LS_Pool_interest() + deposited + borrowed
        #todo: SYS_LS_Pool_interest = SYS_LS_interest* [1-args["treas"]/100]
        #todo: SYS_TR_interest = SYS_LS_interest*[args["treas"]/100]
        #todo: LP_Pool_total_deposited_stable -> SYS_LP_Pool_TV_IntDep_stable = deposited+SYS_LP_Pool_interest
        #todo:
